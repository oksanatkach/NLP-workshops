...
lr = LR()
auc = train_and_eval_auc(lr, train_data_features, train["sentiment"],test_data_features, test["sentiment"].values)
...
rf_aucs = []
for i in range(10):
    auc = train_and_eval_auc(forest, train_data_features, train["sentiment"],test_data_features, test["sentiment"].values)

    print
    "random forest run {}, AUC: {}".format(i, auc)
    rf_aucs.append(auc)
